...
{
   sw_i2c3_init();             // init I2C interface
}

// Reads a single byte from a specified address
uint8_t SW_I2C3ReadByte( uint8_t address, uint8_t * readData )
{
   if( sw_i2c3_start_wait(address+I2C_READ))  // set device address and read mode
		return 1;
	*readData = sw_i2c3_readNak();   // read last byte 
...
uint8_t SW_I2C3ReadBytes( uint8_t address, uint8_t noOfReadBytes, uint8_t * readData )
{
   uint8_t i;
	...
	if( noOfReadBytes )
	{
		...
		for( i = 0; i < noOfReadBytes-1; i++)
			readData[i] = sw_i2c3_readAck();   // read one byte 
		readData[i] = sw_i2c3_readNak();   // read last byte 

    	sw_i2c3_stop();    // set stop condition = release bus
	}
...
{
   if( sw_i2c3_start_wait(address+I2C_WRITE)) // set device address and write mode
		return 1;
   sw_i2c3_write(dataVal); // write data
   sw_i2c3_stop();    // set stop conditon = release bus
...
uint8_t SW_I2C3WriteBytes( uint8_t address, uint8_t noOfWriteBytes, uint8_t * writeData )
{
   int i;
   ...
   if( noOfWriteBytes )
	{
		...
		{
        	sw_i2c3_write(writeData[i]); // write data
		}
    	sw_i2c3_stop();    // set stop conditon = release bus
	}
...
uint8_t SW_I2C3WriteReadBytes( uint8_t address, uint8_t noOfWriteBytes, uint8_t * writeData, uint8_t noOfReadBytes, uint8_t * readData )
{
   ...
   if( noOfWriteBytes )
	...
	{
    	if( noOfWriteBytes )
        	i = sw_i2c3_rep_start(address+I2C_READ);       // set device address and read mode
		else
        	i = sw_i2c3_start_wait(address+I2C_READ);     // set device address and read mode
...
uint8_t SW_I2C3IndexedWriteBytes( uint8_t address, uint8_t noOfIdxBytes, uint8_t *idxVal, uint8_t noOfWriteBytes, uint8_t * writeData )
{
    ...
    if( noOfWriteBytes )
	{
		...
		{
        	sw_i2c3_write(idxVal[i]); // write register index
		}
		for( i = 0; i < noOfWriteBytes; i++ )
		{
        	sw_i2c3_write(writeData[i]); // write data
...
uint8_t SW_I2C3ReadBytesNoWait( uint8_t address, uint8_t noOfReadBytes, uint8_t * readData )
{
	...
	if( noOfReadBytes )
	{
		if( sw_i2c3_start(address+I2C_READ))  // set device address and read mode
		{
			return 1;
		}
		//    	tx = 0;
