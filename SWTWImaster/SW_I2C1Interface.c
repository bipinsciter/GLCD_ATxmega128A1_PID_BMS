...
{
   sw_i2c1_init();             // init I2C interface
}

// Reads a single byte from a specified address
uint8_t SW_I2C1ReadByte( uint8_t address, uint8_t * readData )
{
   if( sw_i2c1_start_wait(address+I2C_READ))  // set device address and read mode
		return 1;
	*readData = sw_i2c1_readNak();   // read last byte 
  	sw_i2c1_stop();    // set stop conditon = release bus

   return 0;
}

// Reads bytes from a specified address
uint8_t SW_I2C1ReadBytes( uint8_t address, uint8_t noOfReadBytes, uint8_t * readData )
{
   uint8_t i;
	...
	if( noOfReadBytes )
	{
		...
		for( i = 0; i < noOfReadBytes-1; i++)
			readData[i] = sw_i2c1_readAck();   // read one byte 
		readData[i] = sw_i2c1_readNak();   // read last byte 

    	sw_i2c1_stop();    // set stop condition = release bus
	}
   return 0;
}
...
{
   if( sw_i2c1_start_wait(address+I2C_WRITE)) // set device address and write mode
		return 1;
   sw_i2c1_write(dataVal); // write data
   sw_i2c1_stop();    // set stop conditon = release bus
   return 0;
}

// Writes bytes to a specified address
uint8_t SW_I2C1WriteBytes( uint8_t address, uint8_t noOfWriteBytes, uint8_t * writeData )
{
   int i;
   if( noOfWriteBytes )
	{
		...
		{
        	sw_i2c1_write(writeData[i]); // write data
		}
    	sw_i2c1_stop();    // set stop conditon = release bus
	}
   return 0;
...
uint8_t SW_I2C1WriteReadBytes( uint8_t address, uint8_t noOfWriteBytes, uint8_t * writeData, uint8_t noOfReadBytes, uint8_t * readData )
{
   ...
   if( noOfWriteBytes )
	...
	{
    	if( noOfWriteBytes )
        	i = sw_i2c1_rep_start(address+I2C_READ);       // set device address and read mode
		else
        	i = sw_i2c1_start_wait(address+I2C_READ);     // set device address and read mode
...
uint8_t SW_I2C1IndexedWriteBytes( uint8_t address, uint8_t noOfIdxBytes, uint8_t *idxVal, uint8_t noOfWriteBytes, uint8_t * writeData )
{
    ...
    if( noOfWriteBytes )
	{
		...
		{
        	sw_i2c1_write(idxVal[i]); // write register index
		}
		for( i = 0; i < noOfWriteBytes; i++ )
		{
        	sw_i2c1_write(writeData[i]); // write data
...
uint8_t SW_I2C1ReadBytesNoWait( uint8_t address, uint8_t noOfReadBytes, uint8_t * readData )
{
	...
	if( noOfReadBytes )
	{
		if( sw_i2c1_start(address+I2C_READ))  // set device address and read mode
		{
			return 1;
		}
		//    	tx = 0;
