...
void ks0108GotoXY(uint8_t x, uint8_t y)
{
    ...
    cmd = LCD_SET_ADD | x;
    ks0108WriteCommand(LCD_ON, chip);              // power on
//    ks0108WriteCommand(LCD_DISP_START, chip);      // display start line = 0
...
void ks0108Init(uint8_t invert)
{
    ...
    LCD_CMD_DIR |= LCD_CMD_DIR_MASK;                             // command port is output
...
uint8_t ks0108DoReadData()
{
    uint8_t data;
    volatile uint8_t i;

//    if( ks0108Coord.x > 64  )                         // chip2 X-address = 0
//       ks0108WriteCommand(LCD_SET_ADD + ks0108Coord.x - 64, CHIP2);
//    else
//       ks0108WriteCommand(LCD_SET_ADD + ks0108Coord.x, CHIP1);
    ...
    // Dummy Enable -- Strange behaviour. Without this does not read correct
    LCD_CMD_PORT |= 0x01 << EN;                     // EN high level width: min. 450ns
    LCD_CMD_PORT &= ~(0x01 << EN);
    for( i=0; i<16; i++ );                           // a little delay loop (faster than reading the busy flag)
...
uint8_t ks0108ReadData(void)
{
    return ks0108DoReadData();                     // "real" read
}
...
//        ks0108WriteCommand(LCD_SET_ADD, CHIP2);
    ...
    {
        // first page
        cmdPort = (LCD_CMD_PORT & LCD_CMD_DIR_MASK);                     // save command port
        displayData = ks0108ReadData();
        // restore command port
        LCD_CMD_PORT &= (cmdPort | ~(LCD_CMD_DIR_MASK)); 
        LCD_CMD_PORT |= cmdPort; 
        LCD_DATA_DIR = 0xFF;                        // data port is output
...
void ks0108Circle(uint8_t xcenter, uint8_t ycenter, uint8_t radius, uint8_t color)
{
    int tswitch, y, x = 0;
    unsigned char d;
    ...
    while (x <= y) 
	{
        ks0108SetDot(xcenter + x, ycenter + y, color);     
		ks0108SetDot(xcenter + x, ycenter - y, color);
        
		ks0108SetDot(xcenter - x, ycenter + y, color);     
		ks0108SetDot(xcenter - x, ycenter - y, color);
        
		ks0108SetDot(ycenter + y - d, ycenter + x, color); 
		ks0108SetDot(ycenter + y - d, ycenter - x, color);
...
void ks0108SetStartLine(uint8_t start)
{
    ks0108WriteCommand(LCD_DISP_START | start, CHIP1);      // display start line = 0
    ks0108WriteCommand(LCD_DISP_START | start, CHIP2);
}
